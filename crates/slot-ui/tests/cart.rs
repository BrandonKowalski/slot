...
fn the_cartridge_is_centred_on_the_row_and_clears_both_the_plate_and_the_slot() {
    ...
    let (top, foot) = (rest_y(CART_H as f32), foot_y(CART_H as f32));
    assert_eq!((top, foot), (172.5, 307.5));
    assert_eq!(top + foot, OUT_H as f32);
    assert!(
        top > PLATE_H,
        "the top edge at {top} is under the HUD plate"
    );
    assert!(
        foot < lip,
